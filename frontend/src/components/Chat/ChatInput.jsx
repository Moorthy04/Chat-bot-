...
const ChatInput = ({
    ...
    // Paste handler — paste image/file directly into input
    ...
    // Fix 3: Drag and drop handlers on outermost wrapper and robust counter
    ...
    }, [text, attachments, isUploading, isResponding, onSend]);
    ...
    return (
        ...
        >
            <LayoutGroup>
                ...
                >
                    {/* Fix 3: Improved drag overlay and child intercept prevention */}
                    <AnimatePresence>
                        {isDragging && (
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                                className="absolute inset-0 rounded-2xl flex items-center justify-center z-50 bg-(--input-bg)/90 border-2 border-dashed border-(--button-primary) pointer-events-none"
                            >
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-12 h-12 rounded-full bg-(--button-primary)/10 flex items-center justify-center text-(--button-primary)">
                                        <Plus size={24} />
                                    </div>
                    ...
                    {/* All content inside container should not intercept drag when isDragging is true */}
                    <div className={cn("flex flex-col w-full", isDragging && "pointer-events-none")}>
                        {/* Attachments */}
                                ...
                                >
                                    {attachments.map((file, i) => (
                                        <motion.div
                                            ...
                                            className="relative group flex items-center gap-2.5 px-3 py-2 rounded-xl bg-(--hover-bg) border border-(--border) w-44 pointer-events-auto"
                                        >
                        ...
                        <textarea
                            ...
                            className="w-full bg-transparent border-none text-(--foreground) outline-none focus:ring-0 resize-none px-4 pt-[10px] pb-2 text-[15px] leading-relaxed placeholder:text-(--foreground)/30 overflow-y-auto scrollbar-none pointer-events-auto"
